...
Qed.
	...
	using the \texttt{tactic} as shown 
	...
	tactic in Coq that introduces the 
	variables into the context, effectively
	...
	\begin{itemize}
		...
		type of Booleans. Such a 
		shallow embedding of $B$
		from our example
		is created as \texttt{f} in 
		Step 2 of Fig.~\ref{fig:smtcoqex}. 
	...
	Given Boolean term $B$, SMTCoq's proof 
	checker checks the deep embedding of 
	$B$ against the proof of $B$ from the 
	...
	proof of it from the SMT solver, if 
	the checker able to check an SMT
