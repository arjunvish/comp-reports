...
\end{minted}
	...
	conversion rule is an 
	important rule that allows a term to 
	...
	\begin{itemize}
		...
		these custom types with respect
		to the shallow types.
	\end{itemize}
	...
	(deep embedding)
	in terms of the Coq library types
	...
	inductive types (deep types) in 
	order to prove their similarity to 
	the shallow terms, using the 
	...
	object from the SMT solver proves 
...
Inductive form : Type :=
	...
	\noindent In keeping with the terminology
	in~\cite{DBLP:phd/hal/Keller13}, an 
	implementation of the \texttt{check} 
	function from the previous section
	is called a \textit{checker}. SMTCoq 
	splits the checker into multiple 
	\textit{small checkers}, and the 
	\textit{main checker} implements the 
	actual \texttt{check} function, by 
	composing the checking done by the 
	small checkers. The main checker
	...
	corresponds to the proof object
	$proof$ from the SMT solver, 
	and returns \texttt{true} if it 
	does. Specifically, \texttt{main\_checker}
	...
	This is because the SMT solver isn't 
	decidable --- given a formula, it 
	could either return \texttt{sat} 
