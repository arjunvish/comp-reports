...
\label{sec:intro}
	...
	to Booleans goals in Coq via a
...
\label{sec:smtproofs}
	...
	$M$, and its complement is added 
	by appending $\neg x$. $F$ is a set of 
	clauses that is equisatisfiable with 
	the input formula. The initial formula
	$F_0$ may be transformed by the algorithm
	but the transformation is always 
	equisatisfiable with the initial input.
	$C$ represents the conflict 
	clause, a clause from $F$ that is 
	falsified by the current assignment 
	(or an equisatisfiable transformation 
	of a clause from $F$) ---
	...
	$\langle (), F_0, \emptyset \rangle$. 
	The final state is one of:
	\begin{itemize}
		\item $fail$, when $F_0$ is 
		unsatisfiable.
		\item $\langle M, F, \emptyset \rangle$ 
		where $F_0$ is satisfiable, $F$ is 
		a transformation of $F_0$ by the 
		algorithm that is equisatisfiable
		with $F$, and $M 
		\models F$.
	\end{itemize}
	...
	rule ($\mathit{Backj}$) uses the 
	conflict clause to undo one or more 
	decisions and add a propagated literal
	to the context. As a special case 
	of this, the newly propagated 
	literal is the complement of a 
	previously assigned (and undone)
	decision.
	...
	decision $b \mapsto \bot$ can be made 
	...
	clause's disjuncts, 
	separately negated. The premise 
	of the $\mathit{Lemma}$ rule 
	is a subproof of $\bot$ from 
	the complements of the disjuncts 
	of the theory lemma, and from this it 
	concludes the lemma. 
	...
	\begin{example}
		...
		this subproof to derive $L$. The application 
	...
	which is an extension of the dependent 
	...
	by referring to 
	them as \textit{kinds}. At the bottom of the 
	...
	\texttt{Set} kind, and (2) propositions or 
	properties in Coq that have kind
	\texttt{Prop}. The rest of the 
	hierarchy looks like this: 
	\texttt{Prop} and 
	\texttt{Set} have kind
	\texttt{Type$_1$} and 
	\texttt{Type$_i$} has kind 
	\texttt{Type$_{i+1}$}. The implementation 
	of propositions as a kind in Coq is its
	means of enforcing the Curry-Howard 
	correspondence between proofs and 
	terms (or programs), and between 
	propositions and types. As a consequence,
	propositions having the \texttt{Prop} 
	kind in Coq, are only provable, if 
	...
	This term can be constructed within 
	...
	\textit{tactics}. Coq implements a 
	constructive logic via its 
	\texttt{Prop} type. Constructive 
...
Qed.
	...
	\texttt{forall (a : Prop), a $\lor$ $\sim$a}
...
Qed.
	...
	\begin{figure}[t]
		...
		\begin{framed}
			...
			is now (assuming predicate symbol $P_i$ 
			corresponds to Boolean predicate symbol 
			$B_i$):
			\begin{center}
...
\texttt{Qed.}
	...
	SMTCoq is 
	able to prove formulas over some
...
\end{minted}
		...
		\noindent An example of an equivalence 
